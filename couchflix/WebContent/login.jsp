...
<script>
		...
		function testAPI() {
			FB.api('/me', function(response) {
				...
				request.open("POST",
						"http://localhost:8080/CouchFlix/index.jsp", true);
				...
				request.send("emailId="
					+ response.email + "&password=password&action=login");
			});
		}
	</script>
	...
	<%
		...
		if ("validate".equals(action)) {
			...
			String check = request.getParameter("checksum");
			UserManager us = new UserManager();
			User_info user = us.readUserByEmail(em);
			user.setEnabled(true);
			isValidation = true;
			us.updateUser(user);
		}

		if ("invalidEmail".equals(action)) {
			invalidEmail = true;
		}
		if ("invalidPassword".equals(action)) {
			invalidPassword = true;
		}
		if ("notEnabled".equals(action)) {
			notEnabled = true;
		}
		if("verifyEmail".equals(action)) {
			verifyEmail = true;
		}
		if("emailExists".equals(action)) {
			emailExists = true;
		}
	...
	<div class="container" align="center" style="padding-top: 70px">
		...
		<form action="index.jsp" method="post" role="form">
			<div class="col-lg-4 col-lg-offset-4">
				<input id="emailId" type="text" placeholder="Your Email?" autofocus
			...
			<div class="col-lg-4 col-lg-offset-4">
				<button type="submit" name="action" class="btn btn-primary"
					value="login">Sign In Now!</button>
			</div>
		</form>
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<a href="lostpassword.jsp">Lost your password?</a>
		</div>
		<%
			if (isValidation) {
		%>
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<h3 style="color: white;">Email ID has been verified!</h3>
		</div>
		<%
			}
		%>
		...
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<h3 style="color: white;">Invalid Email ID!</h3>
		</div>
		<%
			}
		%>
		...
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<h3 style="color: white;">Incorrect Password!</h3>
		</div>
		<%
			}
		%>
		...
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<h3 style="color: white;">Email ID not verified!</h3>
		</div>
		<%
			}
		%>
		...
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<h3 style="color: white;">An email has been sent to the registed email address.</h3>
		</div>
		<%
			}
		%>
		...
		<br> <br>
		<div class="col-lg-4 col-lg-offset-4">
			<h3 style="color: white;">The email address entered already exists.</h3>
		</div>
		<%
			}
		%>
